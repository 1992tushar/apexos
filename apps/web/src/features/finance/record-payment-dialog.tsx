...
import type { InvoiceRow } from "@/lib/dto";
...
export function RecordPaymentDialog({ invoice }: { invoice: InvoiceRow }) {
  ...
  const {
    ...
    defaultValues: { amount_rupees: invoice.balance_minor / 100, method: "bank_transfer" },
  });
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      toast({ title: "Payment recorded", description: invoice.invoice_no, variant: "success" });
      setOpen(false);
      reset();
      router.refresh();
    } catch (err) {
  ...
  return (
    <Dialog
      ...
      onOpenChange={(o) => {
        setOpen(o);
        if (o) reset({ amount_rupees: invoice.balance_minor / 100, method: "bank_transfer" });
      }}
    >
      ...
      <DialogContent className="sm:max-w-[440px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {invoice.invoice_no} · {invoice.customer_name} · Balance {formatMoney(invoice.balance_minor)}
          </DialogDescription>
        </DialogHeader>
        <form
          onSubmit={onSubmit}
